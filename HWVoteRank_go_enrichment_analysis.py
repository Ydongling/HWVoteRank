...
from HWVoteRank.utils import add_genes_as_columns, get_top_20_drivers, get_top_drivers_enriched_terms, get_unique_genes, plot_heatmap, process_go_data, DATA_DIR, OUT_DIR
...
if __name__ == "__main__":
    ...
    go_biological_df_1 = pd.read_csv(join(OUT_DIR, 'CancerDriver', 'Cancer', 'BP_chart_AD906B6ED0231641720483595.txt'), sep='\t')
    go_molecular_df_1 = pd.read_csv(join(OUT_DIR, 'CancerDriver', 'Cancer', 'MF_chart_AD906B6ED0231641720526877.txt'), sep='\t')
    
    go_biological_df = go_biological_df_1.sort_values(by="Count" , ascending=False)
    go_molecular_df = go_molecular_df_1.sort_values(by="Count" , ascending=False)
