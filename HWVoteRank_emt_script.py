...
from HWVoteRank.network_node_importance import build_graph_from_edge_list, compute_pearson_correlation, HWVoteRank
from HWVoteRank.utils import compute_mutation_count, configure_logging, find_coding_noncoding_drivers, DATA_DIR, OUT_DIR, NUM_miR
...
if __name__ =="__main__":
    ...
    F=HWVoteRank(G, 500, 0.01,0.1)
    node_importance_df = pd.DataFrame(F, columns=['node', 'importance']) 
    ...
    noncoding_gold_standard_mes_df = pd.read_excel(join(DATA_DIR, 'EMT', 'EMT-miRNA.xls'))
